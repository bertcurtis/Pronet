...
namespace Pronet.Protractor
{
    ...
    public class NgWebDriver : IWebDriver, IWrapsDriver, IJavaScriptExecutor
    {
        ...
        /// <summary>
        /// If true, the page will not wait for the onload function to complete
        /// </summary>
        public bool IgnoreWaitForDom { get; set; }
        ...
        public string Url
        {
            ...
            set
            {
                ...
                if (hcDriver != null &&
                    (hcDriver.Capabilities.HasCapability("internet explorer") ||
                     hcDriver.Capabilities.HasCapability("MicrosoftEdge") ||
                     hcDriver.Capabilities.HasCapability("phantomjs") ||
                     hcDriver.Capabilities.HasCapability("firefox") ||
                     hcDriver.Capabilities.HasCapability("safari")))
                {
        ...
        public NgWebElement FindElement(By by)
        {
            ...
            this.WaitForAngular();
            return new NgWebElement(this, this.driver.FindElement(by), by);
        }
        ...
        public NgWebElement FindElement(By by, By altBy)
        {
            ...
            {
                ((JavaScriptBy)altBy).AdditionalScriptArguments = new object[] { this.RootElement };
            }
            this.WaitForAngular();
            try
            {
            ...
            catch
            {
                try
                {
                    return new NgWebElement(this, this.driver.FindElement(altBy), altBy);
                }
                catch
                {
                    throw;
        ...
        public ReadOnlyCollection<NgWebElement> FindElements(By by)
        {
            ...
            this.WaitForAngular();
            var collection = new List<NgWebElement>();
            foreach (var element in this.driver.FindElements(by))
            {
                collection.Add(new NgWebElement(this, element, by));
            }
        ...
        public ReadOnlyCollection<NgWebElement> FindElements(By by, By altBy)
        {
            ...
            catch
            {
                try
                {
                    foreach (var element in this.driver.FindElements(altBy))
                    {
                        collection.Add(new NgWebElement(this, element, altBy));
                    }
                    return collection.AsReadOnly();
                }
                catch
        ...
        ReadOnlyCollection<IWebElement> ISearchContext.FindElements(By by)
        {
            ...
            return new ReadOnlyCollection<IWebElement>(this.driver.FindElements(by).ToList());
        }
        ...
        public void WaitForAngular()
        {
            if (!this.IgnoreSynchronization)
            {
                var task = Task.Factory.StartNew(() => this.ExecuteAsyncScript(ClientSideScripts.WaitForAngular));//, this.rootElement));
                if (!task.Wait(TimeSpan.FromSeconds(60)))
                {
                    throw new TimeoutException("Angular did not resolve it's async functions in the given time alotment.");
                }
            }
        }
        ...
        {
            //if (!this.IgnoreWaitForDom)
            //{
            //    var task = Task.Factory.StartNew(() => this.ExecuteAsyncScript(ClientSideScripts.WaitForDomToLoad, this.rootElement));
            //    if (!task.Wait(TimeSpan.FromSeconds(60)))
            //    {
            //        throw new TimeoutException("The DOM did not load in the needed elements in the given time alotment.");
            //    }
